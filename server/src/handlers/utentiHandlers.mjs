...
async function loginUtente(req, res) {
	try {
		...
		if (!utente) {
			return res.status(401).json({ message: "error", errors: [{ field: "username", message: "Utente non esistente" }] });
		}
		...
		if (!isPasswordCorrect) {
			return res.status(401).json({ message: "error", errors: [{ field: "password", message: "Password errata" }] });
		}
