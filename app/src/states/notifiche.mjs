...
async function fetchNotifiche() {
	const response = await axios.get(NOTIFICHE_URL+"utenti/"+loggedUsername, {headers: {'Token': token}});
	notifiche.value = response.data;
}

async function setAllAsRead() {
    await axios.put(NOTIFICHE_URL+"utenti/"+loggedUsername, {} ,{headers: {'Token': token}});
    await fetchNotifiche();
}

async function deleteAll() {
    await axios.delete(NOTIFICHE_URL+"utenti/"+loggedUsername, {headers: {'Token': token}});
    await fetchNotifiche();
}
